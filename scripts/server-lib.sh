...
require_debian_family() {
  ...
  case "${ID:-}:${VERSION_ID:-}" in
    debian:12|ubuntu:22.04|ubuntu:24.04)
      log "sistema suportado detectado: ${PRETTY_NAME:-$ID $VERSION_ID}"
      ;;
    *)
      die "sistema não suportado: ${PRETTY_NAME:-${ID:-desconhecido} ${VERSION_ID:-}}. Suportados: Debian 12, Ubuntu 22.04 LTS e Ubuntu 24.04 LTS."
      ;;
  esac
}
